...
root -l examplesLP/pt_constituents_comp_Top.C'("TTBar_000.root", "TTBarLep_000.root", "ZJetsToNuNu_000.root")'
...
void BookHistograms(ExRootResult *result1, ExRootResult *result2 ,ExRootResult *result3 ,TestPlots *plots)
{
  ...
  plots->fJetPt = result1->AddHist1D(
    "Jet T Contituents Pt JC", "Jet T Contituents Pt JC",
  ...
  plots->fJetPt2 = result2->AddHist1D(
    "Jet T2 Contituents Pt JC", "Jet T2 Contituents Pt JC",
    "p_{T}", "number of jets",600, 0, 1100, 0, 1);
   
  plots->fJetPt2->SetLineColor(kRed);  
  
  plots->fJetPt3 = result3->AddHist1D(
    "Jet T3 Contituents Pt JC", "Jet T3 Contituents Pt JC",
    "p_{T}", "number of jets",600, 0, 1100, 0, 1);
...
{ 
  ...
  cs = new TCanvas("Pt_constituents_Top","Pt_constituents_Top"); 
  
  
  auto hs = new THStack("Jet_pt_constituents","Constituents P_t for Top Jets");
  hs->SetMinimum(1);
  ...
  legend->AddEntry(plots->fJetPt, "t -> bqq' ", "l");
  legend->AddEntry(plots->fJetPt2,"t -> blv ", "l");
  ...
  legend->Draw();
  cs->SetGrid();
  cs->Print("examplesLP/Histograms/Pt_constituents_Top.png");
...
void pt_constituents_comp_Top(const char *inputFile, const char *inputFile2, const char *inputFile3)
{
