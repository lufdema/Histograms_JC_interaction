...
root -l examplesLP/pt_constituents_comp_Higgs.C'("HToBB_000.root", "HToCC_000.root", "HToGG_000.root","HToWW2Q1L_000.root","HToWW4Q_000.root", "ZJetsToNuNu_000.root")'
...
void BookHistograms(ExRootResult *result1, ExRootResult *result2 , ExRootResult *result3, ExRootResult *result4, ExRootResult *result5, ExRootResult *result6, TestPlots *plots)
{
  ...
  plots->fJetPt = result1->AddHist1D(
    ...
    "Pt", "number of jets",600, 0, 1100, 0, 1); //Los ultimos numeros, el cero y 
  ...
  plots->fJetPt2 = result2->AddHist1D(
    ...
    "Pt", "number of jets",600, 0, 1100, 0, 1);
...
void AnalyseEvents4(const char *inputFile, TestPlots *plots)
{
  ...
  for(entry = 0; entry < 100000; ++entry)
  {
    ...
    {	
     ...
     momentum.SetPxPyPzE(px[0][i], py[0][i], pz[0][i], E[0][i]);
     plots->fJetPt4->Fill(momentum.Pt());
    }
...
void AnalyseEvents5(const char *inputFile, TestPlots *plots)
{
  ...
  for(entry = 0; entry < 100000; ++entry)
  {
    ...
    {	
     ...
     momentum.SetPxPyPzE(px[0][i], py[0][i], pz[0][i], E[0][i]);
     plots->fJetPt5->Fill(momentum.Pt());
    }
...
void AnalyseEvents6(const char *inputFile, TestPlots *plots)
{
  ...
  for(entry = 0; entry < 100000; ++entry)
  {
    ...
    {	
     ...
     momentum.SetPxPyPzE(px[0][i], py[0][i], pz[0][i], E[0][i]);
     plots->fJetPt6->Fill(momentum.Pt());
    }
...
{ 
  ...
  auto hs = new THStack("Jet_Pt_Higgs","Constituents P_{t} for Jets coming from Higgs Decays");
  hs->SetMinimum(1);
  ...
  legend->Draw();
  cs->SetGrid();
  cs->Print("examplesLP/Histograms/Pt_Constituents_Higgs.png");
...
void pt_constituents_comp_Higgs(const char *inputFile, const char *inputFile2, const char *inputFile3,const char *inputFile4,const char *inputFile5, const char *inputFile6)
{
