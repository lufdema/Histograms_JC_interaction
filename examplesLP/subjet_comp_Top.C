...
root -l examplesLP/subjet_comp_Top.C'("TTBar_000.root", "TTBarLep_000.root", "ZJetsToNuNu_000.root")'
...
{ 
  ...
  cs = new TCanvas("Jet_Sub_Top","Jet_Sub_Top"); 
  
  
  auto hs = new THStack("Jet_Sub_Top","Jet #tau_{3}/#tau_{2} for Top Jets");
  hs->SetMinimum(1);
  ...
  legend->AddEntry(plots->fJetSub, "t -> bqq' ", "l");
  legend->AddEntry(plots->fJetSub2,"t -> blv ", "l");
  ...
  legend->Draw();
  cs->SetGrid();
  cs->Print("examplesLP/Histograms/sub_Top.png");
...
void subjet_comp_Top(const char *inputFile, const char *inputFile2, const char *inputFile3)
{
